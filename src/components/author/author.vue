            ...
            <div class="listcon"
              ...
              <div class="r_txt">
                <div class="txtbox">
                  ...
                  <span class="aut-list-digest"><i class="vip"
                                                   v-if="item.tabView == undefined && item.chargeable">黑必退</i>{{item.digest}}</span>
                </div>
                <div v-if="item.matches != undefined && item.matches.length" class="match"
                     @click.stop="gomatch(469732)">
            ...
            <div class="aut-room-list" v-for="item in autRoomList" :id="item.roomId" @click="gooRoom(item)">
              ...
              <div infoinner>
                <div class="msg">
                  ...
                  <p class="roompic">门票: <span :class="{on:item.roomStatus==2 || item.roomStatus==1}"
                                               v-html="setPrice(item)"></span></p>
                  <p class="time">直播时间:&nbsp;&nbsp; {{formatTime(item.startTime)}} —— {{formatTime(item.startTime,
                    item.endTime)}}</p>
                  <span
