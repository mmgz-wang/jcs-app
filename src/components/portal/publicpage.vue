    ...
    if (!this.$route.meta.iskeep || this.isFirstEnter) {
      ...
      if (pageName == "足球") {
        ...
        document.getElementsByTagName("title")[0].innerText = "免费";
          ...
          this.reqUrl = '/article/list/allArticles/'
        this.lastArticleId = "";
        this.sportType = '';
        this.headerData.ele = "<h1>全部</h1>";
        document.getElementsByTagName("title")[0].innerText = "全部";
      } else if (pageName == "晒战绩") {
        ...
        this.$http
          ...
          .then(function(res) {
            if (res.data.code == "0000" || res.data.Code == "0000") {
              if (this.types) {
                this.articleDataList = this.articleDataList.concat(
              ...
              this.lastArticleId = this.articleDataList[
                this.articleDataList.length - 1
              ].id - 1;
            } else {
              layer.open({
